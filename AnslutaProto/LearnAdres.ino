...
char AdresByteA = 0x01;
char AdresByteB = 0x01;
...
void loop(){
  ...
  delay(20);
  if (digitalRead(MISO)) {      
    char PacketLength = ReadReg(CC2500_SRXFIFO);
    char recvPacket[PacketLength];
    if(PacketLength <= 8) {
      for(char i = 1; i <= PacketLength; i++){    //Read the received data from CC2500
        recvPacket[i] = ReadReg(CC2500_SRXFIFO);
      }
